...
\begin{frame}[c]
  \begin{center}
    \begin{tikzpicture}[-,> = stealth',node distance=1.8cm, very thick]
      ...
      \visible<2->{
        \node[style={font=\sffamily\Large}] (branch_name) at (3,4) {Name};
      ...
      \visible<3->{
        \node[style={font=\sffamily\Large}] (branch_ref) at (8,0) {Referenz};
        \path[<-,uigray,dotted,thick,shorten <= 8.0pt]
          (master.south) edge [bend left=10] (branch_ref.west);
      }
...
\begin{frame}[c]
  \begin{center}
    \begin{tikzpicture}[-,> = stealth',node distance=1.8cm, very thick]
      ...
      % Head ref
      \visible<3->{
        \node[]  (fake) [right of=master]  {}; % fake node
        \node[head]  (head) [right of=fake]  {head};
